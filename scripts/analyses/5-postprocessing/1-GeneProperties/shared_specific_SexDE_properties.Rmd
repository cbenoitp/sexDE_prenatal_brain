...
### Load constraint and format it in bins
...
Also add the LOEUF bins of the merged sex-DE data.

```{r add_LOEUF_bins, echo=FALSE}
fullMerge_LOEUFbins <- merge(fullMerge, merged_sexDE_results[, c(1,40)], by = "ensemblID", suffixes = c("", "_allSexDE"))
```
...
Also compare the full set of sex-DE genes with the non-sex-DE genes.

```{r expression_prenatal_sexDEVSnonsexDE, echo=FALSE}
...
# compare means
compare_means(AveExpr_prenatal~category, fullMerge)
# plot
p <- ggplot(fullMerge, aes(x = category, y = AveExpr_prenatal, fill = category))
p <- p + geom_violin(alpha = 0.5, scale = "area", linewidth = 0.01, show.legend = FALSE) 
p <- p + stat_summary(fun.data=data_summary, size=0.1, show.legend = FALSE)
p <- p + scale_fill_manual(values = c('non sexDE' = "black", 'sexDE' = "grey"))
p <- p + theme_minimal() + xlab("") + ylab("Prenatal expression")
p <- p + stat_compare_means(method = "wilcox.test",
                            label = "p.signif", hide.ns = TRUE, vjust = 0,
                            tip.length = 1, na.rm = TRUE,
                            label.x = 1.5)
p
# save plot
ggsave("prenatal_expression_sexDE_vs_nonsexDE.png",
       path = "images/5-postprocessing/1-GeneProperties/shared/",
       width = 6, height = 8, units = "cm")
```

```{r expression_adult_sexDEVSnonsexDE, echo=FALSE}
# compare means
compare_means(AveExpr_adult~category, fullMerge)
# plot
p <- ggplot(fullMerge, aes(x = category, y = AveExpr_adult, fill = category))
p <- p + geom_violin(alpha = 0.5, scale = "area", linewidth = 0.01, show.legend = FALSE) 
p <- p + stat_summary(fun.data=data_summary, size=0.1, show.legend = FALSE)
p <- p + scale_fill_manual(values = c('non sexDE' = "black", 'sexDE' = "grey"))
p <- p + theme_minimal() + xlab("") + ylab("Adult expression")
p <- p + stat_compare_means(method = "wilcox.test",
                            label = "p.signif", hide.ns = TRUE, vjust = 1,
                            tip.length = 1, na.rm = TRUE,
                            label.x = 1.5)
p
# save plot
ggsave("adult_expression_sexDE_vs_nonsexDE.png",
       path = "images/5-postprocessing/1-GeneProperties/shared/",
       width = 6, height = 8, units = "cm")
```
...
p <- p + stat_compare_means(method = "wilcox.test", label = "p.signif", label.x = 1.5, hide.ns = TRUE)
p
# save plot
...
Same using the constraint bins computed for all genes analyzed in either the adult or prenatal data.

```{r const_sexDEVSnonsexDE2, echo=FALSE}
# add variable defining if the gene is sexDE in at least one of the 2 dataset
fullMerge_LOEUFbins$category <- ifelse(is.na(fullMerge_LOEUFbins$specificity), "non sexDE", "sexDE")
# compare means
compare_means(LOEUF~category, fullMerge)
# plot
p <- ggplot(fullMerge_LOEUFbins, aes(x = category, y = LOEUF, fill = category))
p <- p + geom_violin(alpha = 0.5, scale = "area", linewidth = 0.01, show.legend = FALSE) 
p <- p + stat_summary(fun.data=data_summary, size=0.1, show.legend = FALSE)
p <- p + scale_fill_manual(values = c('non sexDE' = "black", 'sexDE' = "grey"))
p <- p + theme_minimal() + xlab("") + ylab("Constraint (LOEUF)")
...
p
# save plot
ggsave("constraint_LOEUF_sexDE_vs_nonsexDE_new.png",
       path = "images/5-postprocessing/1-GeneProperties/shared/",
       width = 6, height = 8, units = "cm")
```

Also plot the proportion of sex-DE genes per bin of constraint (bin of LOEUF score).

```{r const_sexDEVSnonsexDE, echo=FALSE}
# format data
fullMerge_LOEUFbins_plot <- fullMerge_LOEUFbins[, c(1,18,19)] %>%
  group_by(LOEUF_bin_allSexDE, category) %>%
  summarise(count = n()) %>%
  ungroup() %>%
  group_by(LOEUF_bin_allSexDE) %>%
  mutate(proportion = count / sum(count))
# plot
p <- ggplot(fullMerge_LOEUFbins_plot, aes(x = LOEUF_bin_allSexDE, y = proportion, fill = category, alpha = category))
p <- p + geom_bar(stat = "identity", position = "fill")  # Stacked bar plot with proportional fill
p <- p + scale_y_continuous(labels = scales::percent_format())  # Show y-axis as percentages
p <- p + labs(x = "Constraint bins (LOEUF)", y = "Gene proportion")
p <- p + scale_fill_manual(values = c('non sexDE' = "black", 'sexDE' = "grey"), name = "")
p <- p + scale_alpha_manual(values = c('non sexDE' = 0.5, 'sexDE' = 0.5), name = "")
p <- p + theme_minimal()
p
# save plot
ggsave("proportion_sexDE per_constraint_LOEUF_bin.png",
       path = "images/5-postprocessing/1-GeneProperties/shared/",
       width = 12, height = 8, units = "cm")
```
...
```{r aveExpr_prenatal_Xchr, echo=FALSE}
# format data
fullMerge_spe_Xchr_escape$AnnotEscape <- ifelse(fullMerge_spe_Xchr_escape$sexDE_prenatal != "ns" | fullMerge_spe_Xchr_escape$sexDE_adult != "ns", "sex-DE", "not sex-DE")
# plot
p <- ggplot(fullMerge_spe_Xchr_escape, aes(x = AnnotEscape, y = AveExpr_prenatal, fill = AnnotEscape))
p <- p + geom_violin(alpha = 0.5, scale = "area", linewidth = 0.01, show.legend = FALSE) 
p <- p + stat_summary(fun.data=data_summary, size=0.1, show.legend = FALSE)
p <- p + theme_minimal() + xlab("") + ylab("Expression in prenatal forebrain")
p <- p + scale_fill_manual(values = c("not sex-DE" = "black", "sex-DE" = "grey"))
p <- p + stat_compare_means(method = "wilcox.test", label = "p.signif", label.x = 1.4)
p
# save plot
ggsave("prenatal_expression_Xchr_escape_sexDE_vs_nonsexDE.png",
       path = "images/5-postprocessing/1-GeneProperties/shared/",
       width = 6, height = 8, units = "cm")
```

```{r aveExpr_adult_Xchr, echo=FALSE}
# plot
p <- ggplot(fullMerge_spe_Xchr_escape, aes(x = AnnotEscape, y = AveExpr_adult, fill = AnnotEscape))
p <- p + geom_violin(alpha = 0.5, scale = "area", linewidth = 0.01, show.legend = FALSE) 
p <- p + stat_summary(fun.data=data_summary, size=0.1, show.legend = FALSE)
p <- p + theme_minimal() + xlab("") + ylab("Expression in adult forebrain")
p <- p + scale_fill_manual(values = c("not sex-DE" = "black", "sex-DE" = "grey"))
...
p
# save plot
ggsave("adult_expression_Xchr_escape_sexDE_vs_nonsexDE.png",
       path = "images/5-postprocessing/1-GeneProperties/shared/",
       width = 6, height = 8, units = "cm")
```
...
overlap_gtex_sexDE_Xchr$isSignif <- ifelse(overlap_gtex_sexDE_Xchr$sexDE_prenatal != "ns" | overlap_gtex_sexDE_Xchr$sexDE_adult != "ns", "sex-DE", "not sex-DE")
p <- ggplot(overlap_gtex_sexDE_Xchr, aes(x = isSignif, y = percent_sexDE, fill = isSignif))
p <- p + geom_violin(alpha = 0.5, scale = "area", linewidth = 0.01, show.legend = FALSE) 
p <- p + stat_summary(fun.data=data_summary, size=0.1, show.legend = FALSE)
p <- p + scale_fill_manual(values = c("not sex-DE" = "black", "sex-DE" = "grey"))
p <- p + theme_minimal() + ylab("Percent of GTEx tissues sex-DE") + xlab("")
p <- p + stat_compare_means(method = "wilcox.test", label = "p.signif", label.x = 1.4)
p
# save
