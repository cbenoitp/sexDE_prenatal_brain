...
Do a barplot with the number of genes in each category, coloring by the direction of sex bias for each bar.

```{r barplot_nbrGenes_perCategory_sexBiasDirection, echo=FALSE}
...
# plot
p <- ggplot(dataToPlot, aes(x = specificity, fill = sexDEsimplified))
p <- p + geom_bar(stat = "count")
...
p
# save plot
ggsave("nbrGenes_specificity_barplot_colorDirection.png", 
       path = 'images/4-GTEx/2-comparison/lineModel/barplot/', 
       plot = p, width = 13, height = 12, units = "cm")
```

Do a similar barplot with the number of genes in each category, coloring by the direction of sex bias for each bar and adding the proportion of the specific category the each direction correspond to.
...
Do similar plots looking at autosomal vs sex-chr genes.
