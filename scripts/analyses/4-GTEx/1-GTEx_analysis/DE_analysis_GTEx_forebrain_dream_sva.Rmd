---
title: "GTEx sex-DE analysis with only svas"
...
# Sex-DE analysis on genes with svas
...
save(x, metadata, file = 'data/4-GTEx/1-GTEx_analysis/analysis_sva/brain_forebrain_v8_dream_counts_phenotypes_sva.RData')
```
...
design <- model.matrix(~SEX+DTHHRDY+TISSUE)
design0 <- model.matrix(~DTHHRDY+TISSUE)

# design for voomDream (the variable to be tested must be a fixed effect)
form <- ~ SEX + (1|DTHHRDY) + (1|TISSUE) + (1|SUBJID)
```

Voom normalization

```{r voom_norm, echo=FALSE}
png('images/4-GTEx/1-GTEx_analysis/analysis_sva/trendplots/brain_forebrain_v8_VoomDream_fit_sva.png')
...
names(metadata) <- c("SEX", "DTHHRDY", "SUBJID", "TISSUE", "SAMPLE", paste0("X", 1:n.sv))
# include svas in formula
form <- reformulate(c("SEX", "(1|DTHHRDY)", "(1|TISSUE)", "(1|SUBJID)", paste0("X", 1:n.sv)))
# Redo voom with new design matrix
y <- voomWithDreamWeights(x, form, metadata, plot = TRUE)
# save the formatted counts and phenotypes as a RData object
save(y, metadata, sva.obj, form,
     file = 'data/4-GTEx/1-GTEx_analysis/analysis_sva/brain_forebrain_v8_dream_counts_voom_phenotypes_sva.RData')
```
...
save(fitmm, file = 'data/4-GTEx/1-GTEx_analysis/analysis_sva/brain_forebrain_v8_fitmm_dream_sva.RData')
```
...
# Write output file
write.table(fitmm2, "data/4-GTEx/1-GTEx_analysis/analysis_sva/brain_forebrain_v8_VoomDream_topTable_with_sva_ind.txt", col.names=TRUE, row.names=F, quote=F)
```
