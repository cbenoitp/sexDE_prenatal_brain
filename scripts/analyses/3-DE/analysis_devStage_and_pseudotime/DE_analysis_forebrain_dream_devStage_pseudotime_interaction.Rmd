---
title: "sex-DE dream analysis on HDBR forebrain samples using pseudotime and developmental stages as covariate"
...
form <- ~ Sex + pseudotime + (1|DevStage) + Sex:pseudotime + (1|Individual)
# design for sva computation
design <- model.matrix(~metadata$Sex + metadata$pseudotime + metadata$DevStage + metadata$Sex:metadata$pseudotime)
```
...
png('images/3-DE/analysis_devStage_and_pseudotime/trendplot/trendplot_VoomDream_forebrain_pseudotime_devStage_ind_interaction.png')
...
names(metadata) <- c("Sex", "pseudotime", "DevStage", "Individual", paste0("X", 1:n.sv))
# include svas in formula
form <- reformulate(c("Sex", "pseudotime", "(1|DevStage)", "Sex:pseudotime", "(1|Individual)", paste0("X", 1:n.sv)))
```
...
save(y, phenotypes, form, sva.obj, metadata,
     file = "data/3-DE/analysis_devStage_and_pseudotime/normCounts_dream_pseudotime_devStage_sva_ind_interaction_phenotypes.RData")
```
...
save(fitmm, file = "data/3-DE/analysis_devStage_and_pseudotime/fitmm_dream_pseudotime_devStage_sva_ind_interaction_phenotypes.RData")
```
...
write.table(fitmm2, 
            "data/3-DE/analysis_devStage_and_pseudotime/VoomDream_topTable_forebrain_pseudotime_devStage_sva_ind_interaction.txt", 
...
write.table(fitmm_interaction, 
            "data/3-DE/analysis_devStage_and_pseudotime/VoomDream_topTable_forebrain_pseudotime_devStage_sva_ind_interaction_term.txt", 
            col.names = TRUE, row.names = FALSE, quote = FALSE)
```
...
write.table(fitmm_pseudotime, 
            "data/3-DE/analysis_devStage_and_pseudotime/VoomDream_topTable_forebrain_pseudotime_devStage_sva_ind_interaction_pseudotime.txt", 
            col.names = TRUE, row.names = FALSE, quote = FALSE)
```
