---
title: "Visualize DE results for analysis using pseudotime (inferred with 100 genes between the 600 and 700 most variable genes)"
...
load("data/2-pseudotime/varInputGenes/forebrain_pseudotime_analysis_700-600genes.RData")
# load sex-DE data
load("data/3-DE/analysis_pseudotime_700-600genes/normCounts_dream_pseudotime_700-600genes_sva_ind_interaction_phenotypes.RData")
de_results <- read.table("data/3-DE/analysis_pseudotime_700-600genes//VoomDream_topTable_forebrain_pseudotime_700-600genes_sva_ind_interaction.txt",
                         header = TRUE)
de_results_interaction <- read.table("data/3-DE/analysis_pseudotime_700-600genes//VoomDream_topTable_forebrain_pseudotime_700-600genes_sva_ind_interaction_term.txt", 
                                     header = TRUE)
de_results_pseudotime <- read.table("data/3-DE/analysis_pseudotime_700-600genes/VoomDream_topTable_forebrain_pseudotime_700-600genes_sva_ind_interaction_pseudotime.txt",
                                    header = TRUE)
```
...
2584/3313 sex-DE genes from this new analysis were also found in the original analysis, representing a large fraction (78.00%) of the 3187 sex-DE genes from the original analysis.
...
11940 genes (82.15%) from this new analysis were also pseudotime-DE in the original analysis.
...
ggsave(namePlot, 
       path = 'images/3-DE/analysis_pseudotime_700-600genes/pvalue_distrib/',
...
ggsave(plot = p, 
       path = 'images/3-DE/analysis_pseudotime_700-600genes/barplot/',
...
ggsave(namePlot, 
       path = "images/3-DE/analysis_pseudotime_700-600genes/volcanoplot/",
...
p
# save plot
png("images/3-DE/analysis_pseudotime_700-600genes/UpsetPlot/complexUpsetPlot_between_sexDE_pseudotimeDE_allGenes.png",
...
ggsave("violinPlot_variancePartition.png", p,
       path = "images/3-DE/analysis_pseudotime_700-600genes/variancePartition/",
       width = 20, height = 12, units = "cm", create.dir = TRUE)
```

# Explore the 100 genes used to infer the pseudotime

For this, we look at the expression of these genes in the different brain cell types. We can also do a quick GO term analysis.

Get the names of the genes.

```{r get_gene_names, echo=FALSE}
selectedGeneNames <- row.names(selectedCounts)
```


## Cell type expression of this gene list
...
Plot the distribution of the expression of these genes in the different brain cell types.

```{r singleCell_expression, echo=FALSE}
# format data
...
cellTypeExprProfile_formated <- cellTypeExprProfile_formated[which(cellTypeExprProfile_formated$gene_name %in% selectedGeneNames), ]
cellTypeExprProfile_formated <- cellTypeExprProfile_formated %>% 
                                    pivot_longer(cols = 1:11, names_to = "CellType", values_to = "meanExpr")
# remove some unwanted cell type
cellTypeExprProfile_formated <- cellTypeExprProfile_formated[which(!cellTypeExprProfile_formated$CellType %in% c("Purk", "Gran")),]
# plot
...
p <- ggplot(cellTypeExprProfile_formated, aes(x = CellType, y = meanExpr))
p <- p + geom_violin(aes(fill = CellType), alpha = 0.5, scale = "area", linewidth = 0.01, show.legend = FALSE) 
...
p
# save plot
ggsave("cellType_expression_geneInference.png", 
       path = "images/3-DE/analysis_pseudotime_700-600genes/",
       width = 10, height = 8, units = "cm")
```

These genes are most highly expressed in interneurons and excitatory neurons.


## GO enrichment analysis

Prepare universe for the GO enrichment analysis.

```{r universe, echo=FALSE}
universe <- y$genes$gene_name
```
...
```{r enrichment_analysis, echo=FALSE}
resGO_BP <- enrichGO(selectedGeneNames, universe = universe, ont = "BP", OrgDb = org.Hs.eg.db, 
                  keyType = "SYMBOL", minGSSize = 15, maxGSSize = 500, pvalueCutoff = 0.05)
print(sprintf(">>> GO BP - number GO term enriched: %i", length(resGO_BP@result$ID)))
resGO_MF <- enrichGO(selectedGeneNames, universe = universe, ont = "MF", OrgDb = org.Hs.eg.db, 
                  keyType = "SYMBOL", minGSSize = 15, maxGSSize = 500, pvalueCutoff = 0.05)
print(sprintf(">>> GO MF - number GO term enriched: %i", length(resGO_MF@result$ID)))
resGO_CC <- enrichGO(selectedGeneNames, universe = universe, ont = "CC", OrgDb = org.Hs.eg.db, 
                  keyType = "SYMBOL", minGSSize = 15, maxGSSize = 500, pvalueCutoff = 0.05)
print(sprintf(">>> GO CC - number GO term enriched: %i", length(resGO_CC@result$ID)))
```

Look at the keywords of the GO terms enriched.

```{r function_count_GO_keyword, echo=FALSE}
countKeyword <- function(resGO, keywords){
  # get only GO term enriched in the choosen cluster
  GOresult <- resGO@result
  # Count number of GO terms containing at least one of the keywords
  containKeyword <- Reduce("|", lapply(keywords, grepl, x = GOresult$Description))
  return(sum(containKeyword))
}
```

```{r count_keywords, echo=FALSE}
print(sprintf(">>> GO BP - cell cycle: %i", countKeyword(resGO_BP, cellCycle)))
print(sprintf(">>> GO BP - development: %i", countKeyword(resGO_BP, development)))
print(sprintf(">>> GO BP - neuro: %i", countKeyword(resGO_BP, neuro)))
print(sprintf(">>> GO BP - rna: %i", countKeyword(resGO_BP, rna)))
```
